...
class Aurum:
    ...
    def _build_system_prompt(self, context: str, memory: str = "") -> str:
        ...
        parts += [
            ...
            "`trade_positions`: id, **trade_group_id** (NOT `group_id`), timestamp, mode, session, ticket, magic_number, direction, **lot_size** (NOT `lot`), entry_price, sl, tp, status, close_price, close_time, close_reason, pnl, pips, tp_stage. There is **no** `open_time` column — the row's `timestamp` is the open time.",
