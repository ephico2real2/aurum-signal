...
- **FORGE native entry quality (v2.6.5+):** Before placing a native scalp, FORGE can reject the setup on **M5** microstructure — ATR floor, average body vs wicks, directional bar count, and BB width expansion. Skips appear in the journal / **`forge_signals`** as **`entry_quality_atr`**, **`entry_quality_body`**, **`entry_quality_direction`**, **`entry_quality_bb_contraction`** (not a signal-room or AURUM failure).
