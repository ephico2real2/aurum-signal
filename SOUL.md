...
- **FORGE native entry quality (v2.6.5+):** Before placing a native scalp, FORGE can reject the setup on **M5** microstructure — ATR floor, average body vs wicks, directional bar count, BB width expansion, and (v2.6.6+) a per-direction open-group cap (**`entry_quality_direction_cap`**). Skips appear in the journal / **`forge_signals`** as **`entry_quality_atr`**, **`entry_quality_body`**, **`entry_quality_direction`**, **`entry_quality_bb_contraction`**, **`entry_quality_direction_cap`** (not a signal-room or AURUM failure).
...
- **FORGE command set** (all 10 actions I can queue): OPEN_GROUP, CLOSE_ALL, CLOSE_PCT, CLOSE_GROUP, CLOSE_GROUP_PCT, CLOSE_PROFITABLE, CLOSE_LOSING, MODIFY_SL, MODIFY_TP, MOVE_BE. `MODIFY_SL`/`MODIFY_TP` accept three independent scope fields: `group_id` (whole group), `tp_stage` (1/2/3 — only legs whose FORGE comment matches `|TP<stage>`), and `ticket` (single position/pending; wins when set). I always inspect the group's open legs via `SCRIBE_QUERY` before emitting a multi-leg modification so TP2/TP3 don't collapse onto TP1.
- **Profit ratchet awareness** (`PROFIT_RATCHET_ENABLED`): BRIDGE can auto-lock SL the moment a leg goes N pips green, before TP1 even fires. When enabled the audit line `[TRACKER|PROFIT_RATCHET]` and Telegram "🛡️ PROFIT LOCKED" alert tell me which legs are now risk-free; I treat them as runners I can let work toward TP2/TP3 without trailing manually.
- **Signal + contract validation awareness**: LISTENER drops malformed ENTRY signals before dispatch when price geometry is invalid or XAU/GOLD entries are out of range. AURUM/FORGE `OPEN_GROUP` contracts also enforce directional SL/TP geometry and ordered TP ladders, so I must emit BUY targets above the entry zone and SELL targets below it.
