...
**v2.7.37 logging extension** (target — see `docs/FORGE_LOGGING_EXTENSION_DESIGN.md` + `docs/FORGE_DECISION_STACK_INVENTORY.md` §6) closes all gaps below.

> **Why this section exists**: Decision Stack Inventory §6 (v2.7.36) made the
> spec-vs-implementation gap concrete — **Layer-4 atoms reference indicators
> that aren't journaled**. Post-mortems can see WHICH gate fired but not the
> indicator value that drove it. The H1 DI gate, daily bias gate, M30 trend
> gate, H4 RSI/ADX gates, and the M5 cascade composites are the worst
> offenders. The atom-level mapping below cites the composite each missing
> indicator feeds, so each line item has a load-bearing reason to be added.
...
### §3.4. Atom-level deficit (from Decision Stack Inventory §6)

These are the indicators **consumed by Layer-4 atoms** but missing from SIGNALS. Each line cites the composite that uses it and the SKIP gate code it would unblock for post-mortem.

| Missing indicator | Atom predicate(s) | Composite(s) | Gate code(s) that lose context today |
|---|---|---|---|
| `h1_di_plus`, `h1_di_minus` | `h1_di_plus < h1_di_minus` | `BB_BREAKOUT_BUY`, `BB_BREAKOUT_SELL` | `entry_quality_h1_di_buy`, `entry_quality_h1_di_sell` |
| `h4_rsi_v`, `h4_adx_v` | `h4_rsi_v in bear/bull band`, `h4_adx_v in band` | `BB_BREAKOUT_BUY`, `BB_BREAKOUT_SELL` | `entry_quality_h4_rsi_{buy,sell}_blocked`, `entry_quality_h4_adx_{buy,sell}_blocked` |
| `h4_trend_strength` | (derivable from h4 ema20/50/atr) | `BB_BREAKOUT_BUY`, `BB_BREAKOUT_SELL` | Lot factor + post-mortem trend alignment |
| `m15_trend_strength` | `m15_trend_strength` magnitude | (HTF alignment heuristics) | (no direct gate today; post-mortem) |
| `m30_trend_strength` | `m30_trend_strength <= 0` | `BB_BREAKOUT_SELL` | `entry_quality_m30_not_bearish` |
| `d1_open`, `d1_close` | `d1_open > d1_close` (bear day), `<` (bull day) | `MOMENTUM_DUMP_{SELL,BUY}` | `dump_d1_bias_block` |
| `day_high`, `day_low` | (atoms TBD for V3 composites) | `BULL_DAY_DIP_BUY_V3`, `INTRADAY_REVERSAL_TO_SELL_V3` | (composite-validation enabler) |
| `m5_open_1`, `m5_high_1`, `m5_low_1`, `m5_close_1` | OHLC patterns: `prev_close > bb_u + buffer`, `prev_close <= bb_l*(1+pct)`, body_pct, wicks | All BB_BREAKOUT / BB_BOUNCE triggers | Trigger-side context for every BB-anchored decision |
| `m5_lh_cascade` (3 LH) | `m5_lh_cascade == 1` | `INTRADAY_REVERSAL_TO_SELL_V3` | Cascade composite validation |
| `m5_hl_cascade` (3 HL) | `m5_hl_cascade == 1` | `BULL_DAY_DIP_BUY_V3` | Cascade composite validation |
| `m5_body_pct` | `m5_body_pct >= body_pct_min` | (pre-trigger entry quality) | `entry_quality_body` |
| `h1_atr`, `h4_atr`, `m15_atr`, `m1_atr` | Underlying ATRs used in trend-strength + SL calcs | All setups | Lot/SL post-mortem + cross-TF volatility analysis |
| `h1_bb_*`, `h4_bb_*` | Outer-band BB context | (potential composites — currently no atom uses them directly) | Future atom enablement |

**Bottom line**: 24 indicators currently consumed by atoms but not journaled.
v2.7.37 SIGNALS-schema extension splits them into Tier A (13 from design doc)
and Tier B (11 atom-driven additions). After v2.7.37 ships, every gate's
input variables are recoverable from `SELECT ... FROM SIGNALS WHERE id = ?`
without re-running the tester — closing the post-mortem feedback loop.

### §3.5. Scribe forge_signals — must mirror
...
#                 m5_hl_cascade, m5_body_pct (Tier A), plus Tier B:
