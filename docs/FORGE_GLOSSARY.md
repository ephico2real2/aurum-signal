...
**See [`FORGE_ICT_SETUPS.md`](FORGE_ICT_SETUPS.md)** — the canonical setup catalog with the full mapping table: new ICT `setup_type` → atom set → indicators → folded legacy `setup_subtype` values + plain-English definitions + ICT-canon citations.
...
## §3 FORGE atom catalog (current — v2.7.136)
...
| **`g_ict_last_atom_ob_broken`** (v2.7.133 / renamed v2.7.136) | IctOrderBlock.mqh | no (any-OB-broken flag) | n/a | `atom_ob_broken` (Cat 4 BREAKER_RETEST atom, weight 3 per §B.8.2) |
| **`g_ict_last_breaker_retest_buy` / `_sell`** (v2.7.133) | IctOrderBlock.mqh | yes (direction-specific) | n/a | `atom_breaker_retest_buy` / `_sell` (Cat 4 atom, weight 3 — direction-suffixed refinement of §B.8.2 `atom_breaker_retest`) |
| **`g_ict_last_breaker_fvg_buy` / `_sell`** (v2.7.133) | IctOrderBlock.mqh | yes (direction-specific) | yes (Cat 4-scoped) | `atom_breaker_fvg_buy` / `_sell` (Cat 4 atom, weight 2 — Cat 4 disambiguation of §B.8.2 `atom_fvg_confluence` which also appears in Cat 2) |
| **`g_ict_last_atom_ob_confluence_buy` / `_sell`** (v2.7.136) | IctOrderBlock.mqh | yes (direction-specific) | yes (Cat 2-scoped) | `atom_ob_confluence_buy` / `_sell` (Cat 2 OTE_RETRACE atom, weight 1 per §B.8.2 — post-Phase 3 dependency, previously stub `score += 0`) |
| `Forge_HasOBConfluence(direction, atr, tolerance_atr)` | IctOrderBlock.mqh | yes | n/a | (lookup helper; populates `atom_ob_confluence_*`) |
...
- **2026-05-16** — **F-β.1 shipped (v2.7.130)**: ICT atoms 3-5 (PULLBACK_IN_OTE / PREMIUM_DISCOUNT_ALIGNED / FVG_ON_REVERSAL_LEG) and category composites 1-3 (MSS_CONT_SCORE / OTE_RETRACE_SCORE / LIQ_SWEEP_REV_SCORE) all flipped to Mode A (compute + log to SIGNALS, no gate). Atom-feature columns + composite score columns now carry real values instead of always-0. BREAKER_RETEST composite remains scaffold-only pending Phase 3 IctOrderBlock.mqh body. Mode B promotion (warning de-rate at score<5) gated on histogram validation per `FORGE_SETUP_ICT_MAP.md §B.8.2` + 9-miss corpus.
- **2026-05-17** — **`docs/FORGE_ICT_SETUPS.md` created** — canonical setup catalog table mapping the 4 ICT-canonical `setup_type` strings to their atom sets, indicators, and folded legacy `setup_subtype` values. Linked from §2 above. Distills `FORGE_SETUP_ICT_MAP.md §B.2 + §B.4 + §B.8.2` into a single one-stop reference for "what setup_type fires in which category and what indicators it uses." Revised legacy fold map per consensus-gate audit (skill §I.15): MA_CROSSOVER retires; BB_BREAKOUT_RETEST + FLAG_PENNANT → Cat 2 OTE; ORB → Cat 3 LIQ_SWEEP.
- **2026-05-17** — **v2.7.136 ICT canonical naming alignment**. Pre-M7 cleanup catching naming drift between v2.7.133/134 implementation and the §B.8.2 atom catalog. Renames: SIGNALS column `atom_breaker_present` → `atom_ob_broken` (matches §B.8.2 Cat 4 weight=3 atom verbatim); global `g_ict_last_breaker_present` → `g_ict_last_atom_ob_broken`. Adds: `atom_ob_confluence_buy` / `_sell` SIGNALS columns + `Forge_HasOBConfluence()` helper in IctOrderBlock.mqh + wires Cat 2 OTE_RETRACE composite score to read it (was stub `score += 0` at IctScoring.mqh:317 marked "post-Phase 3" — now actually post-Phase 3, so wired). OB ring rebuild reordered to fire BEFORE the OTE_RETRACE composite block so atom_ob_confluence_* are fresh when Cat 2 scores. One ring rebuild serves both Cat 4 (BREAKER_RETEST) and Cat 2 (OTE_RETRACE) consumers — single source of truth. SIGNALS placeholder count bump 166 → 168. §3 atom catalog table extended with 5 new BREAKER_RETEST + 1 new OTE_RETRACE atoms (was missing per `feedback_glossary_update_mandate`).
- **2026-05-17** — **v2.7.135 Phase 3 OB env knobs**. 4 hardcoded Forge_RebuildOBRing params promoted to env-tunable: FORGE_ICT_OB_DISPLACEMENT_MIN_ATR (1.5), FORGE_ICT_OB_LOOKBACK_BARS (50), FORGE_ICT_OB_RETEST_TOLERANCE_ATR (0.5), FORGE_ICT_OB_FVG_CONFLUENCE_TOLERANCE_ATR (0.5). Full 5-layer wire per feedback_no_dead_env_vars.
- **2026-05-17** — **v2.7.134 Phase 3b BREAKER_RETEST schema parity**. Mirrors F-β.1 5-layer pattern: SIGNALS schema gets 7 new columns (atom_breaker_present, atom_breaker_retest_buy/_sell, atom_breaker_fvg_buy/_sell, breaker_retest_score_buy/_sell). CREATE TABLE + ALTER TABLE idempotent migrations + JournalRecordSignal INSERT col list + VALUES bind + scribe.py CREATE/ALTER/SELECT/INSERT + placeholder count bump 159 → 166. All-zero when `composite_breaker_retest_score_enabled=0` (default). Composite scores 0-10 per the §B.8.2 first-cut weights (breaker_present 3 + retest_in_progress 3 + fvg_confluence 2 + KZ_favorable 1 + HTF_aligned 1).
- **2026-05-17** — **v2.7.133 Phase 3 OB body**. IctOrderBlock.mqh full body (41-line scaffold → 272 lines): OrderBlockZone struct, 16-slot ring buffer, displacement+previous-opposite-candle+FVG-confirmation detection, broken-state tracking (body-close past extreme), breaker-retest tracking (price within tolerance of broken OB level), FVG-confluence atom. Exports g_ict_last_breaker_* globals consumed by IctScoring.mqh::ComputeCategoryScore(category=4) — was stub returning 0. Closes the gap from F-β.1 (3 of 4 categories scoring) to all 4 categories alive. ICT canon: per WebSearch citations in module docstring (innercircletrader.net / luxalgo.com / atas.net).
- **2026-05-16** — **v2.7.132 ICT zone-leading comment scheme shipped**. New canonical broker-comment shape: `<ZONE>_<ORDER_TYPE>|<CAT>_<DIR>|G<ID>|<TP_OR_LEG>|<KZ_DETAIL>|<CONV>[|<SK_DETAIL>]` (see [`FORGE_ICT_COMMENT_CODES.md`](FORGE_ICT_COMMENT_CODES.md)). 3 zone prefixes (KZ / SK / OFF), 8 order-type codes (MKT / LIMIT / LIMIT_L2 / BUY_STOP_CONT / SELL_STOP_CONT / BUY_LIMIT_RECOV / SELL_LIMIT_RECOV / PRE_TP1_RECOV), 4 ICT category short codes (MSS_CONT / OTE_RETR / LIQ_SWEEP / BRK_RETEST), 1-char direction (B/S), 5 killzone detail codes (ASIA_KZ / LDN_OPEN_KZ / NY_OPEN_KZ / LDN_CL_KZ / NY_PM_KZ) + OFF, 1-char conviction tag (H/M/L/?) derived from composite score, 3 Silver Knife detail codes (LDN_SK / AM_SK / PM_SK). All 11 comment builders in ea/FORGE.mq5 migrated in single ship — legacy SCALP_* family dead code. Helper: `ea/include/Forge/IctComment.mqh::Forge_BuildScalpComment()` + 5 helper functions + OnInit self-test. Consumer at FORGE.mq5:3529 dual-parses new + legacy shapes for in-flight pre-migration positions. Group ID stays decoupled per the §3a v2.7.131 review (Gxxxx 4-digit, BRIDGE 1-4999 / EA-native 5001-9999).
