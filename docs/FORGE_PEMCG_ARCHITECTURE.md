...
- **2026-05-16 — v2.7.129 Mode D RETIREMENT shipped (behavioral)**. PEMCG / UMCG / CVCSM / BB_EXHAUSTION_REVERSAL master enables flipped to default OFF (`umcg_enabled=false` is the kill-switch — when off, PEMCG atom-compute at `FORGE.mq5:7894` is skipped, warning counts forced to 0, all downstream consumers receive 0 → no SKIPs fire, no BB_EXHAUSTION triggers). ICT 3-tier (atoms → category composites → Mode A/B/C gates) becomes the SOLE entry-gating substrate. **Code preserved as historical reference**; v2.7.130 follow-up will delete the 355-reference implementation. Operator can re-enable via `FORGE_GATE_UMCG_ENABLED=1` for diagnostic ablation but NOT recommended for live trading (known asymmetric over-blocks per Run 36 incident). Design rationale + 7-atom lessons + 9 historical incidents preserved in [`docs/FORGE_PEMCG_CVCSM_LESSONS_LEARNED.md`](FORGE_PEMCG_CVCSM_LESSONS_LEARNED.md). Mode D decision context in `~/.claude/projects/-Users-olasumbo-signal-system/memory/project_pemcg_retirement_target.md`.
