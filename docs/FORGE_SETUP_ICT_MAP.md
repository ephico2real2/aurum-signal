...
Appendix A's milestones M7-M11 collapse to **M7-M9** under this minimal set. **Revised 2026-05-17 per consensus-gate audit** (skill §I.15; full per-setup canon check in `refinement-ideas/M7-design/2026-05-17_m7-mss-continuation-fold.md §8b/§8c`; canonical catalog in `docs/FORGE_ICT_SETUPS.md`):

| New milestone | Replaces | Effect |
|---|---|---|
| **M7 — Fold to `MSS_CONTINUATION`** | Old M7 (MSS) + old M10 (DISPLACEMENT) + half of old M11 (TREND_CONTINUATION) | One setup_type covers the 6 setups that actually express MSS displacement: `BB_BREAKOUT`, `GAP_AND_GO`, `MOMENTUM_DUMP_COMPOSITE`, `BB_SQUEEZE`, `GRINDING_SELL`, `NY_SESSION_BEARISH_BREAKOUT_SELL`. Plus **`INSIDE_BAR` (provisional — operator call: keep here as a slow-displacement variant OR retire as bespoke chart pattern)**. Subtype column preserves original-trigger identity for ablation. **Legacy `MOMENTUM_DUMP` is RETIRED** (not folded) per operator decision 2026-05-17 — `MOMENTUM_DUMP_COMPOSITE` is the atom-composed ICT-aligned successor (promoted from `_TEST` in v2.7.121); running both in parallel was a validation scaffold, not the design target. |
| **M8 — Fold to `OTE_RETRACEMENT`** | Old M8 (OTE) + other half of M11 | One setup_type covers retracement-into-zone entries: `BB_BOUNCE`, `BB_LOWER_REVERSION_BUY`, `FIB_CONFLUENCE`, `VWAP_REVERSION`, `BULL_DAY_DIP_BUY`, `TREND_CONTINUATION_BUY`, `FRACTIONAL_SELL_IN_BULL`, `INTRADAY_REVERSAL_SELL`. **Reclassified from old M7**: `BB_BREAKOUT_RETEST` (retest IS retracement-into-zone), `FLAG_PENNANT` (flag IS the OTE retracement; the pole is the MSS displacement). **Retire candidate (R33, post-M8 validation)**: `BB_PULLBACK_SCALP` — BB-band detector is non-canonical (BB not in ICT toolkit). Canonical replacement is `atom_pullback_in_ote` (fib 62-79% MSS swing leg, already wired in IctScoring.mqh since v2.7.123 Mode A). Operator decision 2026-05-17 (Option B): retire bespoke detector once parallel validation confirms canonical equivalent fires equivalent-or-better. Same pattern as MOMENTUM_DUMP v1 → composite. |
| **M9 — Fold to `LIQUIDITY_SWEEP_REVERSAL`** | Old M9 (LIQUIDITY_SWEEP) | `BB_EXHAUSTION_REVERSAL_*`, post-M2/M3 remnants of chart patterns, `ASIA_CAPITULATION_BUY`. **Reclassified from old M7**: `ORB` (IPDA opening-range break IS a liquidity sweep — `tradethepool.com/technical-skill/ict-market-structure-shift/` distinguishes session-range break from MSS-continuation). |
| **RETIRE bucket** (separate sub-ship — v2.7.137a tech-debt OR bundled with M7 code) | n/a — superseded or not in any ICT category | (1) `MA_CROSSOVER` — ICT explicitly rejects moving-average crossovers as primary triggers. Citations: [chartinglens.com](https://chartinglens.com/blog/ict-trading-strategy-guide), [eplanetbrokers.com](https://eplanetbrokers.com/training/ict-trading-strategy-explained). Delete the trigger site + the env knob; no migration. (2) `MOMENTUM_DUMP` — superseded by `MOMENTUM_DUMP_COMPOSITE` (atom-composed, ICT-aligned, v2.7.121 promotion from `_TEST`). Operator decision 2026-05-17: parallel validation done — commit to the composite, retire the legacy. Delete the 3 fire sites (FORGE.mq5:13259, 13439, 13572) + the `FORGE_DUMP_*` env knobs that the composite doesn't reuse; no migration. |

(BREAKER_RETEST stays gated behind Phase 3 — M5 in Appendix A is unchanged.)

After M9 the EA has **4 setup_types** (MSS_CONTINUATION / OTE_RETRACEMENT / LIQUIDITY_SWEEP_REVERSAL / BREAKER_RETEST), not 6. Subtype column preserves attribution for ablation studies.

**Revised M7 scope summary** (6 keep + 1 provisional + 3 reclassify + 2 retire, vs the original spec's 11):

| Bucket | Count | Setups |
|---|---|---|
| KEEP in M7 (MSS_CONT) | 6 | `BB_BREAKOUT`, `GAP_AND_GO`, `MOMENTUM_DUMP_COMPOSITE`, `BB_SQUEEZE`, `GRINDING_SELL`, `NY_SESSION_BEARISH_BREAKOUT_SELL` |
| Provisional in M7 (operator call) | 1 | `INSIDE_BAR` |
| Reclassify M7 → M8 | 2 | `BB_BREAKOUT_RETEST`, `FLAG_PENNANT` |
| Reclassify M7 → M9 | 1 | `ORB` |
| RETIRE | 2 | `MA_CROSSOVER` (not ICT-canonical), `MOMENTUM_DUMP` (superseded by `MOMENTUM_DUMP_COMPOSITE`) |
...
- **2026-05-17** — **`docs/FORGE_ICT_SETUPS.md` created** — canonical post-M7 setup catalog distilling this doc's §B.2 + §B.4 + §B.8.2 into a single mapping table (new ICT `setup_type` → atom set → indicators → folded legacy `setup_subtype` + ICT-canon citations). Revised M7-M9 fold targets per consensus-gate audit (skill §I.15): `MA_CROSSOVER` → RETIRE (ICT explicitly rejects MAs); `BB_BREAKOUT_RETEST` + `FLAG_PENNANT` → M8 OTE_RETRACEMENT (retest/flag IS retracement entry by definition); `ORB` → M9 LIQUIDITY_SWEEP_REVERSAL (IPDA opening range break is liquidity sweep). M7 scope reduced from 11 → 7-8 setups. WebSearch citations in `FORGE_ICT_SETUPS.md §6 changelog`.
- **2026-05-17** — **§B.4 revised (R17)** to reflect the consensus-gate audit. Old §B.4 listed all 11 setups as M7 folds (the pre-canonical version). Updated text: M7 now lists 7 keep + 1 provisional (`INSIDE_BAR`), M8 row gains the 2 reclassifications (`BB_BREAKOUT_RETEST`, `FLAG_PENNANT`), M9 row gains `ORB`, plus a new RETIRE bucket for `MA_CROSSOVER`. Cross-references to the M7 design doc `refinement-ideas/M7-design/2026-05-17_m7-mss-continuation-fold.md §8b/§8c` (per-finding canon check + per-setup ICT classification) and the canonical catalog `docs/FORGE_ICT_SETUPS.md`. Doc-only — no code or schema change. Closes prerequisite blocker for the v2.7.138/v2.7.139 M7 code ship per `refinement-ideas/improvement-recommendations/INDEX.md` R17.
- **2026-05-17** — **§B.4 RETIRE bucket expanded** per operator decision: `MOMENTUM_DUMP` (the legacy v1) is RETIRED alongside `MA_CROSSOVER`. Reason: `MOMENTUM_DUMP_COMPOSITE` is the atom-composed ICT-aligned successor (promoted from `_TEST` in v2.7.121 — see FORGE.mq5:1215 comment). Running BOTH in parallel was a validation scaffold, not the design target. The composite already wins on §I.8 plug-and-play criteria (atom-decomposed = pure-function evaluators per principle #1; consistent with the SUM3API modular design vision). M7 fold scope reduced from 7 keep → 6 keep; retire bucket grows from 1 → 2. EA-side retirement: delete the 3 `MOMENTUM_DUMP` fire sites (FORGE.mq5:13259, 13439, 13572) + the `FORGE_DUMP_*` env knobs the composite doesn't reuse. No migration to `setup_subtype` (the composite preserves the displacement semantics on its own). Lands either as v2.7.137a tech-debt sub-ship alongside `MA_CROSSOVER` removal, OR bundled with M7 code in v2.7.138.
- **2026-05-17** — **§B.4 M8 row gets a retire-candidate annotation** for `BB_PULLBACK_SCALP` (R33). Operator decision (Option B response to "is `BB_PULLBACK_SCALP` valid?" question): the BB-band detector is non-canonical (Bollinger Bands aren't in the ICT toolkit). Canonical replacement is `atom_pullback_in_ote` (fib 62-79% MSS swing leg) — already wired in `IctScoring.mqh` since v2.7.123 in Mode A (log only). M8 ships as Option A first (fold `BB_PULLBACK_SCALP` to `OTE_RETRACEMENT_BUY/SELL` setup_type with `setup_subtype = "bb_pullback_scalp"`); then post-M8 validation compares `atom_pullback_in_ote` fire rate vs `BB_PULLBACK_SCALP` fire rate. When the canonical detector confirms equivalent-or-better behavior, retire `BB_PULLBACK_SCALP` entirely (delete sites at FORGE.mq5:12279 BUY + 12373 SELL + `g_sc.pullback_scalp_*` knobs). Same "parallel validation → operator decision" pattern as MOMENTUM_DUMP v1 → composite retirement. EA-side retirement does NOT happen in v2.7.137/v2.7.138 — it's a v2.7.140+ candidate, sequenced after M8 OTE fold lands.
