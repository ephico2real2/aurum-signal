...
- **Profit ratchet** (BRIDGE 1.5.1+): set `PROFIT_RATCHET_ENABLED=true` in `.env` to auto-lock SL on green legs. **Trader-style XAU pip = `$0.10`** (matches SCRIBE `pips` column). Tunables: `PROFIT_RATCHET_TRIGGER_PIPS=15` (fire at +$1.50 unrealised), `PROFIT_RATCHET_LOCK_PIPS=10` (SL lands `$1.00` past entry). Worked example for BUY entry `4620.50`: price reaches `4622.00` → SL pinned at `4621.50`. Watch with `tail -f logs/bridge.log | grep PROFIT_RATCHET`.
