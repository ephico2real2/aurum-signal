...
- New env vars: `PROFIT_RATCHET_ENABLED` (default false, opt-in), `PROFIT_RATCHET_TRIGGER_PIPS` (default 30 → $0.30 on XAU), `PROFIT_RATCHET_LOCK_PIPS` (default 5 → $0.05 past entry; auto-clamped to `< trigger`). Pip convention follows `_pip_size_for_symbol` (XAU = 0.01, JPY = 0.01, majors = 0.0001).
