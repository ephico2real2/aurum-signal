...
### Phase 2 — v2.7.44 (introduce `g_regime` ALONGSIDE existing variables) ✅ SHIPPED 2026-05-12

**Status**: SHIPPED in v2.7.44 (originally targeted v2.7.37, delayed 7 releases). Struct + populator added; zero behavior change.
Implementation: `ea/FORGE.mq5` `struct RegimeState` (~33 LOC, 16 fields per §3 + §11.3) +
`void RegimeUpdate(...)` (~62 LOC) called from `CheckNativeScalperSetups()` after per-tick locals
are computed. All legacy globals (`g_regime_label`, `g_daily_*`, etc.) stay; struct mirrors them.

Two NEW fields computed (not just mirrored):
- `intraday_label` — M5+M15 derived: TREND_BULL / TREND_BEAR / RANGE / DECLINING / RISING
- `intraday_counter_htf` — TRUE when intraday counters HTF (Apr 8 PM gap closer)

Tests: `tests/api/test_forge_27x_gates.py::test_regime_state_struct_declared` + `::test_regime_update_function_called`.
...
### §11.4 Killzone-as-atom in composite specs ✅ SHIPPED v2.7.51 (3 of 5)

**Status**: 3 of 5 refinements SHIPPED v2.7.51 (2026-05-13). BLOCK_SELL_IN_CHOP needs no change (already always-on). CHOP_LADDER_BUY_GRID is skipped — composite doesn't exist in EA yet.

Each Tier 1 composite (per `FORGE_COMPOSITE_ROADMAP.md`) gets a killzone-aware refinement. All 3 shipped knobs default to no-change behavior (1.0 / 0); operator opts in via `.env`. Each refinement reads `g_regime.killzone` and `g_regime.minutes_into_kz` (Phase 2 RegimeState populates these every tick from EA broker clock — matches `forge_signals.killzone` for retrospective validation).

| Composite                  | Killzone gating / amplification                                             | Knob (env name) | Default | Status |
|----------------------------|-----------------------------------------------------------------------------|-----------------|---------|--------|
| `BULL_DAY_DIP_BUY`         | ×N lot amplifier in prime window (`NY_OPEN_KZ` ∪ `LONDON_CLOSE_KZ`)         | `FORGE_AMPLIFY_BULL_DAY_DIP_BUY_PRIME_FACTOR` | 1.0 (off) | ✅ v2.7.51 |
| `INTRADAY_REVERSAL_SELL`   | Only fire amplifier inside `NY_OPEN_KZ` ∪ `LONDON_CLOSE_KZ`                 | `FORGE_GATE_INTRADAY_REVERSAL_REQUIRE_PRIME_KZ` | 0 (off) | ✅ v2.7.51 |
| `MOMENTUM_DUMP_SELL`       | Block in first 60 min of `LONDON_OPEN_KZ` (Judas Swing risk)                | `FORGE_GATE_DUMP_JUDAS_WINDOW_BLOCK`            | 0 (off) | ✅ v2.7.51 |
| `BLOCK_SELL_IN_CHOP`       | Always-on regardless of killzone                                            | — | — | n/a |
| `CHOP_LADDER_BUY_GRID`     | Disable inside `LONDON_CLOSE_KZ` (institutions square — reversal risk)      | — | — | ⏳ composite not yet in EA |

**Gate codes** registered in `gate_legend.json` v2.7.51:
- `dump_judas_window` (category: Session / Time)

The intraday_reversal_require_prime_kz refinement is implemented inside `IsIntradayReversalSellActive()` so the amplifier silently doesn't apply when the gate fires (no separate SKIP code — the composite is a lot multiplier on existing setups, not a setup trigger). The bull_day_dip_buy_prime_amplifier is similarly silent — just multiplies into the lot factor when the KZ matches.

Tests: `tests/api/test_forge_27x_gates.py::test_bull_day_dip_buy_prime_amplifier_wired`, `::test_intraday_reversal_require_prime_kz_wired`, `::test_dump_judas_window_block_wired`.

### §11.5 Per-killzone trade caps ✅ SHIPPED v2.7.46

**Status**: SHIPPED v2.7.46 (2026-05-12). Originally scoped for v2.7.38; landed 8 releases later.

**Knob**: `FORGE_GATE_KILLZONE_MAX_TRADES` (env) → `session_filter.killzones_max_trades_per_kz` (JSON, int 0-99). Default **0 = disabled** (operator opt-in only). 3-5 is the typical operator-recommended setting.

**Gate code**: `killzone_trade_cap` (category: `Session / Time`) — emitted via `JournalRecordSignal("SKIP", ...)` once per M5 bar (throttled via the existing `g_scalper_last_sesswarn_log_bar` marker shared with `session_off` to avoid log spam).

**Helper**: `bool ScalperKillzoneCapOK()` returns false only when:
1. `killzones_enabled` (master toggle), AND
2. `killzones_max_trades_per_kz > 0` (operator-set cap), AND
3. an active killzone label is in effect, AND
4. `g_scalper_killzone_trades >= killzones_max_trades_per_kz`.

**Counter**: `g_scalper_killzone_trades` is incremented alongside `g_scalper_session_trades` on every successful entry (after `opened > 0`), so the cap sees fresh state on the next tick. Counter resets on killzone transition and on daily reset — both already in place since v2.7.36.

**Activation**: Operator sets `FORGE_GATE_KILLZONE_MAX_TRADES=5` (or similar) in `.env`. The gate is otherwise invisible — no behavior change at default 0.

Tests: `tests/api/test_forge_27x_gates.py::test_killzone_trade_cap_wired_end_to_end`.
...
### §11.7b Authority model — EA is the source of truth (v2.7.49 + v2.7.50)

**Rule**: MT5 is where orders fire. The EA's broker-clock-anchored time labels are the only clock that has a causal relationship to a trade. Every Python consumer derives from the EA's view, not the other way around. v2.7.49 established this for **killzones**; v2.7.50 extended the same model to the broader **session** labels (LONDON / NY / LONDON_NY / SYDNEY / ASIAN / OFF).

**Implementation**:

#### Killzone (shipped v2.7.49)

| Layer | Source | Stale-fallback |
|---|---|---|
| **`forge_signals.killzone`** + retrospective analysis | EA (per-signal `JournalRecordSignal` INSERT) | n/a — signals already past |
| **Bridge `_killzone()`** | `get_ea_killzone(MARKET_FILE)` reading `forge_session_state.killzone` from market_data.json | `get_current_killzone_utc()` only when market_data.json missing or `>MT5_STALE_SEC` (default 60s) — throttled WARN log on fallback |
| **athena_api `/api/live` `killzone`** | Bridge state (which mirrors EA) | n/a |
| **athena_api `/api/live` `killzone_local_check`** | Bridge's own UTC-clock compute (`get_current_killzone_utc()`) — kept ONLY as a divergence indicator | n/a |
| **Dashboard render** | `D.killzone` primary; `D.killzone_local_check` shown in **red** when it disagrees with `D.killzone` (DST cutover, broker skew, or broken EA) | Amber fallback indicator when EA value missing |
| **Window definitions (`kz_*_start_min`)** | `config/scalper_config.json:session_filter.kz_*_*_min` (EA's canonical config — same file the EA reads). Mirrored by `trading_session._kz_window()` | `SESSION_KZ_<NAME>_START_MIN`/`_END_MIN` env vars (test override); hard-coded NY-minute defaults |

#### Session (shipped v2.7.50)

| Layer | Source | Stale-fallback |
|---|---|---|
| **`forge_signals.session`** + retrospective analysis | EA (per-signal `JournalRecordSignal` INSERT) | n/a — signals already past |
| **Bridge `_session()`** | `get_ea_session(MARKET_FILE)` reading `forge_session_state.label` from market_data.json | `get_trading_session_utc()` only when market_data.json missing or `>MT5_STALE_SEC` — throttled WARN log on fallback |
| **Bridge `trading_session_label` (SCRIBE row)** | `self._current_session` or fresh `_session()` (both EA-anchored) | UTC fallback inside `_session()` |
| **athena_api `/api/live` `session`** | Bridge state (which mirrors EA) | n/a |
| **athena_api `/api/live` `session_local_check`** | Bridge's own UTC-clock compute (`get_trading_session_utc()`) — divergence indicator only | n/a |
| **aurum.py LENS prompt context** | `get_ea_session(MARKET_FILE)` directly (aurum has its own MARKET_FILE constant) | `get_trading_session_utc()` |
| **Dashboard render** | `D.session` primary; `D.session_local_check` shown in **red** when it disagrees | Amber fallback indicator with `?` suffix when EA missing |
| **herald.py startup banner** | Still `get_trading_session_utc()` — runs at cold-start before EA is up; UTC-clock fallback is the correct semantic here | n/a |

**What this catches**:
- **DST cutover divergence** (3 weeks/yr): EA's EU-DST rules + Python's IANA US-DST rules differ on ~6 days. Dashboard renders the bridge UTC value in red — operator sees it immediately.
- **Operator config drift**: Window minutes live in ONE place (`scalper_config.json`). Editing `SESSION_KZ_*` env vars no longer affects EA/Bridge agreement.
- **Broken EA feed**: `market_data.json` stale → bridge falls back to UTC compute + WARN log + amber dashboard indicator. Operator notices rather than silently trading on stale state.

**What stays decoupled** (intentionally):
- EA's KZ computation logic itself — still uses broker offset + manual DST in Approach B. Strategy Tester needs this; live can't depend on bridge being up.
- `/api/health` `killzone_utc` field — kept as bridge's own heartbeat (clear that it's a local-clock check, not source-of-truth).
...
| 2026-05-13 | **v2.7.52 — KZ warmup gate (per arongroups.co stop-hunt research)**: new `FORGE_GATE_KZ_WARMUP_MIN` env knob (int 0-60, default 0=disabled) + `kz_warmup_min` JSON field in `session_filter` + early-gate dispatch check + `kz_warmup` gate code in `gate_legend.json` (Session / Time category). When knob > 0 AND a killzone is active AND `g_regime.minutes_into_kz < kz_warmup_min`, ALL entries are blocked with SKIP "kz_warmup" (throttled to once per M5 bar via the shared `g_scalper_last_sesswarn_log_bar`). Implements the article-recommended pattern: first 5-15 min of session opens have wide spreads + frequent stop hunts; wait for the first M15 candle to close before entering. Applies uniformly to all killzones — combine with v2.7.51 `FORGE_GATE_DUMP_JUDAS_WINDOW_BLOCK` for the wider 60-min LONDON_OPEN_KZ Judas-specific rule. Test: `test_kz_warmup_gate_wired`. 107/107 PASS. |
| 2026-05-13 | **v2.7.51 — §11.4 killzone-aware composite refinements (3 of 5 SHIPPED)**: BULL_DAY_DIP_BUY prime-window amplifier (`FORGE_AMPLIFY_BULL_DAY_DIP_BUY_PRIME_FACTOR`, default 1.0 = disabled) multiplies lot factor by N when killzone is NY_OPEN_KZ ∪ LONDON_CLOSE_KZ (the 60-70%-of-daily-range gold prime window). INTRADAY_REVERSAL_SELL prime-KZ restriction (`FORGE_GATE_INTRADAY_REVERSAL_REQUIRE_PRIME_KZ`, default 0) — when on, the V3 SELL amplifier only applies inside the prime window. MOMENTUM_DUMP Judas-window block (`FORGE_GATE_DUMP_JUDAS_WINDOW_BLOCK`, default 0) — when on, blocks SELL with new gate code `dump_judas_window` when killzone=LONDON_OPEN_KZ AND minutes_into_kz<60 (Judas Swing reversal risk). All 3 default to no-change; operator opts in. Uses `g_regime.killzone` + `g_regime.minutes_into_kz` from Phase 2 RegimeState. BLOCK_SELL_IN_CHOP needs no change (already always-on). CHOP_LADDER_BUY_GRID skipped — composite doesn't exist in EA yet. Tests: 3 new (106/106 PASS). |
| 2026-05-13 | **v2.7.50 — EA-anchored session authority** (§11.7b extended). Same shape as v2.7.49 but applied to the broader session label (`forge_session_state.label` in market_data.json — matches `forge_signals.session` per-signal). `trading_session.get_ea_session()` mirrors `get_ea_killzone()`. `bridge._session()` reads EA-first with throttled-WARN UTC fallback. `bridge.py:2635` SCRIBE-row write now goes through `_session()` instead of `get_trading_session_utc()` directly. athena_api `/api/live` renames `session_utc` → `session_local_check` (divergence-detection field only). aurum.py:434 reads `get_ea_session(MARKET_FILE)` so LENS prompt context sees the EA's authoritative session. Dashboard flips precedence (`D.session` primary, `D.session_local_check` red-on-divergence). herald.py:445 startup banner deliberately stays on `get_trading_session_utc()` — runs before EA is necessarily up. Tests: 7 new in `test_trading_session_ea_anchored.py` (file freshness branches + label-vs-killzone independence). 114/114 PASS. |
| 2026-05-13 | **v2.7.49 — EA-anchored killzone authority model** (new §11.7b). Operator decision: MT5 is where orders fire, so EA's broker-clock killzone is the only clock with causal authority over trades. Python consumers now derive from EA's view: `trading_session.get_ea_killzone()` reads `forge_session_state.killzone` from market_data.json (with `MT5_STALE_SEC` freshness check + throttled WARN on fallback); `bridge._killzone()` swapped to use it; `athena_api /api/live` renames the bridge UTC-clock compute to `killzone_local_check` (clear semantic: integrity check, not source); dashboard flips precedence to EA-first and renders bridge UTC value in red on divergence (DST cutover or broker skew detection). Window definitions (`kz_*_start_min`) unified to a single source: `scalper_config.json:session_filter.*` (EA-authoritative); `_kz_window()` reads scalper_config first, env override second, hard-coded defaults third. Tests: 11 new in `tests/api/test_trading_session_ea_anchored.py` (file freshness branches + window-config precedence branches). 96/96 PASS. |
| 2026-05-12 | **v2.7.48 — backtest_compare killzone + RegimeState breakdowns**: `_run_stats()` now returns `taken_by_killzone` dict (TAKEN counts per KZ label), `htf_h1_strong_rate_pct` (% of TAKEN with override-bypass active), `intraday_counter_htf_rate_pct` (% of TAKEN counter to HTF — the §3.1 Apr-8-PM-gap indicator), and `judas_window_taken` (count of TAKEN inside first 60 min of LONDON_OPEN_KZ). `compare_runs()` extended with `killzone_diff` (mirror of `gate_diff` shape) + 3 new delta keys. All new queries soft-degrade on old aurum_tester.db files that lack the v2.7.45/.47 columns (try/except returns None/{}). New unit-test file: `tests/api/test_backtest_compare.py` (5 tests) using an in-memory sqlite shim — the first Python tests for backtest_compare.py. Run-vs-run can now answer "did Run A enter more in the Judas window than Run B?" — the analytical purpose of every §11 column. 85/85 PASS. |
| 2026-05-12 | **v2.7.47 — RegimeState surfacing to SIGNALS + athena_api gap-closure**: 3 NEW computed regime fields from g_regime now persist to the SIGNALS journal so retrospective composite validation can query them — `htf_h1_strong INTEGER`, `intraday_label TEXT`, `intraday_counter_htf INTEGER` (the §3.1 "intraday vs HTF gap" tracker — composites can `WHERE intraday_counter_htf=1` to find Apr-8-PM-class entries). EA: 3 ALTER TABLE + INSERT column list extension + value emit sourcing from g_regime. scribe.py: CREATE TABLE + 3 ALTER migrations + `has_regime_v47` all-or-nothing source detection + INSERT propagation (column indices shift v37→r[37], v37g3→r[61]). athena_api: `/api/backtest/run/:id` TAKEN entries SELECT now returns `killzone` + `minutes_into_kz` (closes the v2.7.45+v2.7.36 visibility gap — DB had the data, API was hiding it). Tests: 3 new (80/80 PASS). The dashboard render of these new columns is deferred — operator can curl the API directly until the UI is wired. |
| 2026-05-12 | **v2.7.46 — §11.5 per-killzone trade cap SHIPPED**: new `FORGE_GATE_KILLZONE_MAX_TRADES` env knob (int 0-99, default 0=disabled) + `killzones_max_trades_per_kz` JSON field in `session_filter` section + `ScalperKillzoneCapOK()` helper + gate wiring after the session-blocked check in `CheckNativeScalperSetups` + counter increment alongside `g_scalper_session_trades++` on successful entry + `killzone_trade_cap` gate code registered in `gate_legend.json` (category: Session / Time). Original §11.5 spec called for default 3-5 but ship-default is 0 (operator opts in via `.env`) — matches the "additive, no behavior change without explicit knob" pattern of every regime-taxonomy ship. Throttled SKIP emission (once per M5 bar) reuses the existing `g_scalper_last_sesswarn_log_bar` marker. Tests: 1 new (`test_killzone_trade_cap_wired_end_to_end`). 77/77 PASS. |
