...
5a. **Google MQL5/MT5 docs BEFORE asserting any platform fact** (foundational, 2026-05-16 per `feedback_google_mql5_before_assumptions`). Before stating ANY MT5/MQL5 specific as fact — field limits (`MqlTradeRequest.comment` length), function returns, struct widths, retcode meanings, broker behaviors, valid parameter ranges — WebSearch the official mql5.com source FIRST and cite. Training-data memory is unreliable for fast-moving platform details. If docs are silent, say so explicitly; don't invent a number. This applies to docstrings, design docs, code comments, AND verbal claims in chat. Companion to rule 5 (which mandates research for DESIGN choices) — 5a mandates verification for FACTUAL ASSERTIONS.
